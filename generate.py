...
def Generate_Brain():
    ...
    # Create synapses with random weights in the range [-1, 1]
    for sensor_id in sensor_neurons.keys():
        for motor_id in motor_neurons.keys():
            weight = random.uniform(-1, 1)  # Random weight between -1 and 1
            pyrosim.Send_Synapse(sourceNeuronName=sensor_id, targetNeuronName=motor_id, weight=weight)
